 ...
 * ==========================================================
 *
 * Adds clock-hour artist protection in front of the existing
 * request.php endpoint without changing the proven request,
 * queue and ETA logic.
 */
...
{
    $artist = trim($artist);
    $artist = preg_replace('/\s+/u', ' ', $artist) ?? $artist;

    if (function_exists('mb_strtolower')) {
        return mb_strtolower($artist, 'UTF-8');
    }

    return strtolower($artist);
}
...
function srGatewayPublicSongsFile(): ?string
{
    ...
    /*
     * The website normally uses a local relative catalog URL.
     * Do not make an outbound HTTP request from the request endpoint.
     */
...
function srGatewayFindTrackArtist(int $trackId): ?string
{
    ...
    foreach ($songs as $song) {
        ...
        if (
            !is_int($songTrackId) &&
            !(
                is_string($songTrackId) &&
                ctype_digit($songTrackId)
            )
        ) {
            continue;
        }
...
function srGatewayEnsureStateDirectory(): void
{
    ...
    if (
        ...
        throw new RuntimeException(
            'Request state directory could not be created.'
        );
    }
}
...
function srGatewayReserveArtist(
    string $artist,
    ...
    try {
        ...
        if (
            !isset($state['artists']) ||
            !is_array($state['artists'])
        ) {
            $state['artists'] = [];
        }

        $hourKey = $submittedAt->format('Y-m-d-H');
        $normalizedArtist = srGatewayNormalizeArtist($artist);
        $artistKey = hash('sha256', $normalizedArtist);

        /* Remove artist reservations from older clock hours. */
        foreach ($state['artists'] as $storedKey => $storedEntry) {
            if (
                !is_array($storedEntry) ||
                ($storedEntry['hour_key'] ?? '') !== $hourKey
            ) {
...
function srGatewayReleaseArtistReservation(
    string $artistKey,
    string $token
): void {
    try {
        ...
        error_log(
            'Song Request artist reservation cleanup error: ' .
...
$artistProtectionEnabled =
    !defined('ARTIST_HOURLY_PROTECTION') ||
    ARTIST_HOURLY_PROTECTION === true;
...
if (
    ...
    $input = is_string($rawBody)
        ? json_decode($rawBody, true)
        : null;

    $trackIdValue = is_array($input)
        ? ($input['track_id'] ?? null)
        : null;
    ...
    if (
        ...
        if ($trackId > 0) {
            ...
            try {
                $reservation = srGatewayReserveArtist(
                    $artist,
                    $submittedAt
                );
            } catch (Throwable $exception) {
                error_log(
                    'Song Request artist protection error: ' .
                    $exception->getMessage()
                );
            ...
            if (($reservation['allowed'] ?? false) !== true) {
                ...
                srGatewayJson(
                    ...
                    false,
                    'A song by ' . $artist .
                    ' has already been requested this hour. ' .
                    'Please try again after ' .
                    $nextHour->format('H:i') . '.',
                    [
                        'artist' => $artist,
                        'next_artist_request_time' =>
                            $nextHour->format('H:i'),
                        'next_artist_request_at' =>
                            $nextHour->format(DATE_ATOM),
            ...
            /*
             * request.php exits after emitting JSON. Keep the reservation
             * only when that JSON reports a successful request. If the core
             * endpoint rejects the request (IP cooldown, track cooldown,
             * RadioBOSS error, etc.), remove our temporary reservation.
             */
            ob_start();
            ...
            register_shutdown_function(
                static function () use (
                    ...
                    if (
                        ...
                        srGatewayReleaseArtistReservation(
                            $artistKey,
                            $reservationToken
                        );
                    }
